...
function [ IMG ] = mosaic( IM, img , H, x_min, y_min )
    ...
    for n = 1 : width        
        ...
        for c = 1 : C            
            ...
            if (1 <= x && x <= N && 1 <= y && y <= M)    
                ...
                IMG(m,n,c) = (1 - fx) * (1 - fy) * img(fix(y),fix(x),c) +...       
                    (1 - fx) * fy * img(ceil(y),fix(x),c) +...                     
                    fx * (1 - fy) * img(fix(y),ceil(x),c) +...                            
                    fx * fy * img(ceil(y),ceil(x),c);             
            end
        end
    end
end
